...
| `push_sms_truncation.py` | Cắt nội dung SMS của push (CONN-04 bước 5b) ở phía sinh, đo trên envelope đã mã hóa, và đầu vào của năm vector cắt trong `push-envelope.json` |
...
| `verify_push_truncation_checks.py` | Mọi push `sms/new` được cắt lại từ nội dung trước khi cắt, độ dài `env_b64` tính bằng số học thay vì mã hóa; các vector cắt phải đi qua mọi nhánh, biên 1 000, một đoạn cắt ngoài ASCII và một đoạn cắt sau code point ngoài BMP |
