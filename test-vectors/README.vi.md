...
| Khung `HR` (0.4.3) | `device_id` = 16 byte thô; relay chỉ đổi 16 byte đó (đích → nguồn) và chỉ kiểm header; lớp bọc văn bản giữ nguyên từng byte của `env`, bỏ `from` do thiết bị gửi và phát lại dạng `{"from":…,"env":…}`; khung và lớp bọc sai dạng được trả bằng op `error` `BAD_REQUEST` của relay | 0.4.3 cho bố cục nhưng không nói cách xử lý lỗi, cũng không nói có serialize lại `env` hay không |
...
- `kind = "envelope"` (cặp 2, client là iPhone của `relay-auth.json`): các trường của một vector mã hóa trong `envelope.json` với khóa `k_push` (`v`, `type`, `id`, `ts`, `aad`, `aad_hex`, `plaintext`, `nonce`, `ciphertext`, `tag`, `payload_b64`, `envelope`), thêm `sender_device_id` (điện thoại), `recipient_device_id` (iPhone), `env_b64` = base64 chuẩn có padding của chuỗi UTF-8 `envelope`, `reason`, `push_request` (thân `POST /v1/push`), `apns_payload` (thứ relay gửi APNs: `loc-key` = `push.<reason>`, `hl` = `env_b64`, `p` = `pair_id`, `thread-id` chung `sms` hoặc `calls`) và `apns_headers`. `collapse_key` (cũng là `apns-collapse-id`) là `message_key` của SMS (`sms:12847`) và `call:<call_id>` cho cuộc gọi; `ttl_s` là 86 400 cho `sms_new` và 30 cho `call_incoming`. Nội dung: một `sms/new` tiếng Việt, một `sms/new` từ tên người gửi không có trong danh bạ và không có SIM, một `call_event/state` đổ chuông (`time-sensitive`), rồi năm vector cắt SMS bên dưới. Plaintext của push không bao giờ có `local_id`.
...
- Cắt nội dung SMS (CONN-04 bước 5b, SMS-02 API 2 logic 3), chốt bằng các envelope có đối tượng `truncation`. Độ dài tính theo code point Unicode, không bao giờ theo đơn vị UTF-16; cut(text, m) = text khi có tối đa m code point, ngược lại là m − 1 code point đầu và "…". Nội dung dài hơn 1 000 code point thành cut(body, 1000) (`body_to_1000`); khi `env_b64` còn dài hơn 3 000 ký tự, nội dung thành cut(body, m) dài nhất vừa đủ, với m nhỏ hơn độ dài hiện tại (`body_to_fit`); khi ngay cả "…" cũng không vừa, nội dung giữ "…" và snippet được cắt theo cùng cách (`body_to_ellipsis`, `snippet_to_fit`). Bản thân snippet là nội dung mới nhất cắt còn 160 code point, không có "…" (SMS-01). `truncation`: `original_body` và `original_snippet` (trước khi cắt), `steps`, `body_code_points`, `body_utf16_units`, `snippet_code_points`, `env_b64_length`, và sau bước vừa khít có `one_more_code_point_env_b64_length` (lớn hơn 3 000: đoạn cắt là dài nhất còn vừa). Các trường hợp: nội dung tiếng Việt cắt còn 1 000 rồi cắt tiếp cho vừa (818 code point), nội dung ASCII cắt ở 1 000, nội dung đúng 1 000 code point được giữ nguyên, emoji ở chỗ cắt 1 000 (1 000 code point = 1 002 đơn vị UTF-16), và một cuộc trò chuyện nhóm đông (42 địa chỉ) có nội dung giảm còn "…" trước khi cắt snippet. Plaintext là JSON UTF-8 gọn (không escape ký tự ngoài ASCII) nên độ dài như nhau trên mọi nền tảng; bộ kiểm tính lại chúng bằng số học, không mã hóa lại.
...
- `kind = "text_rewrite"`: `outbound` `{"to", "env"}` (khóa theo thứ tự bất kỳ, có thể có khoảng trắng) thành `inbound` = `{"from":"<bên gửi>","env":<env>}` với `env` giữ nguyên từng byte như khi nhận (`env`); một ca dạng gọn, một ca `env` đứng trước, có khoảng trắng và `env` thụt dòng, và một ca thiết bị gửi kèm `from` (`spoofed_from`) mà relay bỏ qua như mọi trường lạ (0.5.1 quy tắc 6), không bao giờ tin.
- `invalid_vectors` kèm lỗi relay phải trả (`expected_error`): khung (`kind = "frame"`) — `bad_magic`, `unsupported_version`, `unknown_op`, `truncated` (19 byte; có header mà không có khung HL) → `BAD_REQUEST`; lớp bọc văn bản (`kind = "text"`) — `bad_wrapper` (`to` không phải device_id, thiếu `env`) → `BAD_REQUEST`, `not_paired` (`to` = chính bên gửi) → `NOT_PAIRED`. Relay chỉ kiểm 20 byte header và có dữ liệu theo sau; không bao giờ xem khung HL (zero-knowledge), việc đó là của thiết bị nhận (0.5.2).
